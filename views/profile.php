<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        $allowed = array('png','PNG', 'jpg','JPG', 'gif','GIF', 'jpeg','JPEG', 'bmp','BMP', 'tif','TIF', '');
        $image_name = $_FILES['image']['tmp_name'];
        $image_tmp = $_FILES['image']['name'];
        $image_ext = pathinfo($image_tmp, PATHINFO_EXTENSION);

        if($image_name == "" || !(in_array($image_ext,$allowed)))
            $img = "";
        else
            $img = addslashes(file_get_contents($image_name));
    ...
    <div class="row">
        ...
        <div class="text-center">
            <?php
                ...
                {
                    echo '
                      <img src="data:image/png;based64,'.base64_encode($userImg).'" class="avatar img-circle img-thumbnail" alt="avatar" />  
                    ';
                }
                else{
                    echo '
